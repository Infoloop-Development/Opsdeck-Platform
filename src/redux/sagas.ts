...
function* userLogin({ payload }: any) {
  try {
    ...
    const response = yield call(api.post, '/api/superadmin/auth/login', {
      email,
      password,
    });
...
function* fetchUserInfoSaga({ payload }) {
  ...
  try {
    const response = yield call(api.get, '/api/users?currentUser=true', options);
    if (!token) {
...
function* getPlansData() {
  try {
    const response = yield call(api.get, '/api/stripe/plans');
